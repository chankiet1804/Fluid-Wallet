...
/// Phrase field, live validation, and the import button.
///
...
///
/// The phrase lives in the field's controller and is cleared on dispose. It
/// never reaches a provider, a route, or a log.
class ImportPhraseForm extends ConsumerStatefulWidget {
  const ImportPhraseForm({
    super.key,
    required this.onImported,
    this.expand = false,
  });

  /// Where to go once the wallet exists — a full screen navigates, a sheet
  /// closes itself.
  final VoidCallback onImported;

  /// Pushes the button to the bottom of the available space. Suits a screen;
  /// a sheet wants to hug its content instead.
  final bool expand;

  @override
  ConsumerState<ImportPhraseForm> createState() => _ImportPhraseFormState();
}

class _ImportPhraseFormState extends ConsumerState<ImportPhraseForm> {
  ...
  @override
  void dispose() {
    // Clears the phrase out of the field's buffer when leaving.
    _controller.clear();
    _controller.dispose();
    super.dispose();
  }
